...
class Collective extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(ENV.REACT_APP_BACKEND + '/api/getMemes?limit='+this.state.limit).then((response)=>{
      this.setState({
        memes:response.data,
    ...
    if (state.loadMore%10 === 0 && state.memes.length>0 && state.memeStatus) {
      ...
      loadMore = (
        <div className='collective-end'>
          You have reached the end of collective.
        </div>
      )
    } else {
      loadMore = null;
...
export default connect(mapStateToProps)(Collective);