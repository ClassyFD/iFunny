...
class Tags extends Component {
  ...
  likeMeme(id) {
    ...
    if (props.user && state.likeArr && !state.likeArr.includes(id) && state.clickable) {
      ...
      axios.post(ENV.REACT_APP_BACKEND+'/api/tagLike/'+id, {user:props.user, type:state.limit, tag:props.match.params.tag}).then((response)=>{
        this.setState({
          likeArr:state.likeArr.concat(id),
          clickable:true,
          memes:response.data
      ...
      axios.post(ENV.REACT_APP_BACKEND+'/api/tagUnlike/'+id, {user:props.user, type:state.limit, tag:props.match.params.tag}).then((response)=>{
        this.setState({
          likeArr:state.likeArr.filter((el, i)=>{
            return el!==id
          }),
